...
$(function () {
    $('#container').highcharts({
        ...
        xAxis: {
            categories: [
            <?php
            $sql = "SELECT * FROM pdc_tarifa_hm";
            $result = mysqli_query($connection,$sql);
            while ($registros = mysqli_fetch_array($result))
            {
            ?>
                '<?php echo $registros["theyearhm"] ?>',
                <?php
            }
            ?>
            ]
        ...
        series: [{
            ...
            data: [
            <?php
            $sql = "SELECT  `theyearhm`, SUM(`consumohm`) AS CONTHM, `costohm`, `idserviciohm`, `serviciohm` FROM pdc_tarifa_hm
            WHERE (YEAR(`theyearhm`) >= 2011 AND YEAR(`theyearhm`) <= YEAR(NOW()))
            GROUP BY YEAR(theyearhm)
            ORDER BY YEAR(theyearhm) ASC";
            $result1 = mysqli_query($connection,$sql);
            while ($registros1 = mysqli_fetch_array($result1))
            {
            ?>
                <?php echo $registros1["CONTHM"] ?>,
                <?php
            }
            ?>
            ],
