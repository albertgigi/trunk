...
$(function () {
    $('#container').highcharts({
        ...
        series: [{
            ...
            color: "#cdad00", //Se define el color de la columna
            data: [
            <?php
            $sql = "SELECT FORMAT((v2gmgaskgcapitayear), 2) AS GKGGCY FROM gmpdcfactorgeiallv2";
            //$sql = "SELECT FORMAT((gaskgcapitayear), 2) AS gkggcy FROM pdc_factor_gei_all";
            $result1 = mysqli_query($connection,$sql);
            while ($registros1 = mysqli_fetch_array($result1))
            {
            ?>
                <?php echo $registros1["GKGGCY"] ?>,
                <?php
            }
            ?>
            ],
