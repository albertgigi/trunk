...
$(function () {
    $('#container').highcharts({
        ...
        xAxis: {
            categories: [
            <?php
            $sql = "SELECT * FROM pdc_tarifa_hm_gpo";
            $result = mysqli_query($connection,$sql);
            while ($registros = mysqli_fetch_array($result))
            {
            ?>
                '<?php echo $registros["theyearhmgpo"] ?>',
                <?php
            }
            ?>
            ]
        ...
        series: [{
            ...
            $sql = "SELECT REPLACE(FORMAT((`consumohmgpo`/1000000), 2),',','') AS CONSHMGPO FROM pdc_tarifa_hm_gpo";
            $result1 = mysqli_query($connection,$sql);
            while ($registros1 = mysqli_fetch_array($result1))
            {
            ?>
                <?php echo $registros1["CONSHMGPO"] ?>,
                <?php
            }
            ?>
            ],
...
<div class="table-responsive">
  <table class="table table-bordered table-hover">
    ...
    <tbody>
    <?php if($loadtarhmgpo): ?>
      <?php foreach($loadtarhmgpo as $item): ?>
      <tr>
        <td><?php echo $item->theyearhmgpo; ?></td>
        <td><?php echo number_format($item->consumohmgpo); ?></td>
        <!--<td><php echo $item->consumohmgpo; ?></td>-->
      </tr>
      <?php endforeach; ?>
    <?php else: ?>
      <tr>
