...
<div class="buscador">
	...
	<select
			id="dep_gas_final_x_mepo"
			name="dep_gas_final_x_mepo"
			class="form-control"
		>
			<option value="">----</option>
		<?php foreach($catdepgsfinal as $item): ?>
			<option value="<?php echo $item->dep_gas_final_x_mepo; ?>">
				<?php echo $item->cta_gas_final_x_mepo. " - ".$item->dep_gas_final_x_mepo; ?>
				</option>
			><?php echo $item->dep_gas_final_x_mepo; ?></option>
		<?php endforeach; ?>
		</select>
</div>
...
<div>
		<h3>
	<?php if($resdepgsfinal1): ?>
		<?php foreach($resdepgsfinal1 as $item): ?>
			<tr>
				<td><?php echo $item->dep_gas_final_x_mepo;
				break; ?>
	...
	<div id = "container" style = "width:100%; height: 400px; margin: 0 auto"></div>
      <script language = "JavaScript">
      	$(document).ready(function() {
            ...
            var chart = {
				colors: ["#cc1c0d", "#cc1c0d" , "#cc1c0d","#cc1c0d", "#cc1c0d" , "#cc1c0d","#cc1c0d", "#cc1c0d" , "#cc1c0d","#cc1c0d", "#cc1c0d" , "#cc1c0d"],
               type: 'column'
            };
            var title = {
               text: ''   
...
<table id="datatables" class="table table-condensed table-bordered table-hover" style="display: none;">
		...
		<tbody>
			<?php if($resdepgsfinal1): ?>
				<?php foreach($resdepgsfinal1 as $item): ?>
			<tr>
				<td><?php echo $item->yer_gas_final_x_mepo; ?></td>
...
<table id="" class="table table-condensed table-bordered table-hover">
		...
		<?php if($resdepgsfinal1): ?>
			...
			<tr class="active">
				<td><b>Total</b></td>
				<td class="text-right"><b><?php echo number_format($EneroTotalGS);?></b></td>
				<td class="text-right"><b><?php echo number_format($FebreroTotalGS);?></b></td>
				<td class="text-right"><b><?php echo number_format($MarzoTotalGS);?></b></td>
				<td class="text-right"><b><?php echo number_format($AbrilTotalGS);?></b></td>
				<td class="text-right"><b><?php echo number_format($MayoTotalGS);?></b></td>
				<td class="text-right"><b><?php echo number_format($JunioTotalGS);?></b></td>
				<td class="text-right"><b><?php echo number_format($JulioTotalGS);?></b></td>
				<td class="text-right"><b><?php echo number_format($AgostoTotalGS);?></b></td>
				<td class="text-right"><b><?php echo number_format($SeptiembreTotalGS);?></b></td>
				<td class="text-right"><b><?php echo number_format($OctubreTotalGS);?></b></td>
				<td class="text-right"><b><?php echo number_format($NoviembreTotalGS);?></b></td>
				<td class="text-right"><b><?php echo number_format($DiciembreTotalGS);?></b></td>
			</tr>
		<?php else: ?>
			<tr>
				<td colspan=10>Favor de seleccionar la dependencia a consultar.</td>
