<?php
Class Modelo_consumo_gas Extends CI_Model {
...
function sumaM3GasPorServicioDependencia($dependencia,$num_servicio,$year){
...
function sumaM3GasPorVariosServicios($array,$year){

		$sql = "SELECT SUM(consumo) AS CONSUMO FROM pdc_consumo_gas";
...
function sumaCostoGasPorServicioDependencia($dependencia,$num_servicio,$year){
...
function sumaCostoGasPorVariosServicios($array,$year){

		$sql = "SELECT SUM(costo) AS CONSUMO FROM pdc_consumo_gas";
...
function sumaM3GasPorYear($year){
    $sql = "SELECT SUM(consumo) AS CONSUMO FROM pdc_consumo_gas";
...
function sumaCostoGasPorYear($year){
    $sql = "SELECT SUM(costo) AS COSTO FROM pdc_consumo_gas";
...
function crear() {
	...
	$this->db->insert("pdc_consumo_gas", $data);
}
...
function editar($id) {
	...
	$data['costo'] = $this->input->post('costo');
	$this->db->where('id', $id);
	$this->db->update("pdc_consumo_gas", $data);
}

/* Borrar un recibo */
function borrar($id) {
	$this->db->where('id', $id);
	$this->db->delete('pdc_consumo_gas');
}

/* Busqueda de un recibo */
...
/*function resultado($limite=false, $inicio=0){
	...
	if(empty($dependencia) && empty($num_servicio)  && !empty($year)){
		...
		$busqueda	= $this->modelo_consumo_gas->servicio_busqueda();
			if(!empty($busqueda)){
				if($limite)
					$this->db->limit($limite, $inicio);
...
function resultadoVariosServicios($array,$year){
		...
		for($i=0; $i < count($array); $i++){
			...
			$sql = "SELECT * FROM pdc_consumo_gas";
...
function servicio_crear() {
	...
	$this->db->insert("pdc_servicios_gas", $data);
}
...
function servicio_editar($id) {
	...
	$data['dependencia'] = $this->input->post('dependencia');
	$this->db->where('id', $id);
	$this->db->update("pdc_servicios_gas", $data);
}

function servicio_borrar($id) {
	$this->db->where('id', $id);
	$this->db->delete('pdc_servicios_gas');
}
...
function ultimos_meses($type, $unidad, $service, $year=false, $month=false, $meses=12 ) {
	...
	$prueba= $this->modelo_consumo_gas->prueba_query($year, $service);
...
function sumaTotalTotalGas($year)
{
	$sql=
	"SELECT CONCAT('', FORMAT(SUM(consumo), 0)) AS SumaConsumo,
	CONCAT('', FORMAT(SUM(costo),2)) AS SumaCosto,
	GROUP_CONCAT(DISTINCT(YEAR(periodo_fin))) AS SumaYear FROM pdc_consumo_gas
