...
"#@%+=FEFGT6R3987EFDF86347GR=+%@#"			theyear AS show_years,
...
"#@%+=FEFGT6R3987EFDF86347GR=+%@#"	$sql.= " GROUP BY theyear ORDER BY theyear";
	...
	function catalogo_gei($limite = false, $inicio = 0)
		{
		if ($limite) $this->db->limit($limite, $inicio);
	...
	*/
	function creacione_factore()
		{
		$this->db->trans_start();
		$start1 = "CREATE TABLE IF NOT EXISTS `pdc_factor_gei_all` ( `id` INT NOT NULL AUTO_INCREMENT ,"#@%+=FEFGT6R3987EFDF86347GR=+%@#"
...
"#@%+=FEFGT6R3987EFDF86347GR=+%@#"	$this->db->query($start1);
		$this->db->query("INSERT INTO `pdc_factor_gei_all` (id, theyear)
"#@%+=FEFGT6R3987EFDF86347GR=+%@#"	VALUES (1,2011), (2,2012), (3,2013), (4,2014), (5,2015), (6,2016), (7,2017)
		ON DUPLICATE KEY UPDATE id = VALUES(ID),
		theyear = VALUES(theyear),
		rawkw = VALUES(rawkw),
	    rawm3wtr = VALUES(rawm3wtr),
	    rawm3gas = VALUES(rawm3gas)");
"#@%+=FEFGT6R3987EFDF86347GR=+%@#"	/*Este query insertara valores para las gráficas de electricidad*/
		$this->db->query("INSERT INTO pdc_factor_gei_all(theyear, rawkw)
"#@%+=FEFGT6R3987EFDF86347GR=+%@#"		(SELECT GROUP_CONCAT(DISTINCT(YEAR(a.periodo_fin))),
...
"#@%+=FEFGT6R3987EFDF86347GR=+%@#"	/*Este query insertara valores para las gráficas de agua*/
		$this->db->query("INSERT INTO `pdc_factor_gei_all`(theyear, rawm3wtr)
"#@%+=FEFGT6R3987EFDF86347GR=+%@#"		(SELECT GROUP_CONCAT(DISTINCT(YEAR(b.periodo_fin))),
...
"#@%+=FEFGT6R3987EFDF86347GR=+%@#"	/*Este query Insertara valores para las gráficas de gas*/
		/*CONSULTA ANTIGUA*/
		...
		$this->db->query("INSERT INTO `pdc_factor_gei_all`(theyear, rawm3gas)
"#@%+=FEFGT6R3987EFDF86347GR=+%@#"		(SELECT GROUP_CONCAT(DISTINCT(YEAR(periodo_inicio))),
			CONCAT(SUM(consumo))
			...
			ON DUPLICATE KEY UPDATE theyear = VALUES(theyear), rawm3gas = VALUES(rawm3gas)");
"#@%+=FEFGT6R3987EFDF86347GR=+%@#"	$this->db->trans_complete();
		}
...
PRIMARY KEY (`id`)) ENGINE = InnoDB";
		...
		$this->db->query("INSERT INTO `factor_gei_agua_comparativo`(`id_servicio`, `year_water_comp`,
"#@%+=FEFGT6R3987EFDF86347GR=+%@#"	`consumo_water_comp`,`costo_water_comp`, `cuenta_waters_comp`)
		(SELECT
		a.servicio,
		GROUP_CONCAT(DISTINCT(YEAR(a.periodo_fin))),
		SUM(a.consumo),
...
"#@%+=FEFGT6R3987EFDF86347GR=+%@#"	$startwat2 = "UPDATE `factor_gei_agua_comparativo`
	SET `pot_waters_comp` = 'POTPOTWATER'
	WHERE `id_servicio` = '44'";
"#@%+=FEFGT6R3987EFDF86347GR=+%@#"	$this->db->query($startwat2);
		$startwat3 = "UPDATE factor_gei_agua_comparativo
"#@%+=FEFGT6R3987EFDF86347GR=+%@#"SET res_waters_comp = 'RESRESWATER'
	WHERE id_servicio = '64' ";
"#@%+=FEFGT6R3987EFDF86347GR=+%@#"	$this->db->query($startwat3);
		$this->db->trans_complete();
		}
...
"#@%+=FEFGT6R3987EFDF86347GR=+%@#"			`theyearhmgpo` YEAR NOT NULL ,
...
"#@%+=FEFGT6R3987EFDF86347GR=+%@#"	$this->db->query($starthmgpo);
...
"#@%+=FEFGT6R3987EFDF86347GR=+%@#"	$this->db->query("INSERT INTO `pdc_tarifa_hm_gpo` (theyearhmgpo, consumohmgpo, costohmgpo)
